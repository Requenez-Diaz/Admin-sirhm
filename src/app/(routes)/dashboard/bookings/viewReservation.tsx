      ...
      <DialogContent className='w-[95vw] max-w-3xl max-h-[90vh] overflow-y-auto p-0 rounded-2xl bg-gray-50 dark:bg-slate-950'>
        <DialogHeader className='px-6 pt-6 pb-4 bg-white dark:bg-slate-900 border-b dark:border-gray-800'>
          <div className='flex justify-between items-start'>
            ...
            <Badge variant={statusVariant as any}>{reservation?.status}</Badge>
          </div>
        </DialogHeader>
        ...
        {loading || !reservation ? (
          ...
          <div className='p-6 space-y-6'>
            ...
            <div className='space-y-4'>
              ...
              {reservation.roomDetails?.length > 0 ? (
                reservation.roomDetails.map((room, index) => (
                  <div
                    key={index}
                    className='bg-white dark:bg-slate-900 rounded-xl border dark:border-gray-800 p-5 shadow-sm flex flex-col sm:flex-row gap-6'
                  >
                    ...
                    <div className='flex-1 space-y-2 text-sm'>
                      <div className='flex justify-between items-start'>
                        <h4 className='font-bold text-base dark:text-gray-100'>
                          {room.name}
                        </h4>
                        <Badge
                          variant='secondary'
                      ...
                      <div className='grid grid-cols-2 gap-y-1 gap-x-4 mt-2'>
                        ...
                        <div className='flex justify-between'>
                          ...
                          <Badge
                            variant='outline'
                            className={`font-medium ${room.seasonName === "ALTA" ? "text-orange-600 border-orange-200 bg-orange-50" : "text-emerald-600 border-emerald-200 bg-emerald-50"}`}
                          >
            ...
            <div className='bg-orange-50 dark:bg-orange-900/10 rounded-xl border border-orange-100 dark:border-orange-900/30 p-5 flex justify-between items-center'>
              <span className='text-orange-800 dark:text-orange-400 font-semibold text-lg'>
                Total Final
              </span>
            ...
            <div className='flex flex-col sm:flex-row gap-3'>
                ...
                !reservation.isInvoiced && (
                  <Button
                    onClick={() => {
                      setOpen(false);
                      router.push(
                        `/dashboard/invoices/new?reservationId=${reservation.id}`,
                      );
                    }}
              ...
              {reservation.isInvoiced && (
                <div className='flex-1 flex items-center justify-center bg-gray-200 dark:bg-gray-800 rounded-xl py-4 px-2 text-sm text-gray-500 font-medium'>
                  <ReceiptText className='w-4 h-4 mr-2' /> Reservación ya
                  facturada
                </div>
              )}
